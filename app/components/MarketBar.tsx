...
import { Ticker as TickerType } from "../utils/types";  // Rename import to avoid conflict
        ...
        <div>
            <div className="flex items-center flex-row relative w-full overflow-hidden border-b border-slate-800">
                <div className="flex items-center justify-between flex-row no-scrollbar overflow-auto pr-4">
                    <TickerComponent market={market} /> {/* Updated to new component name */}
                    <div className="flex items-center flex-row space-x-8 pl-4">
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className={` text-sm font-medium tabular-nums leading-5 text-sm text-greenText ${Number(ticker?.priceChange) > 0 ? "text-green-500" : "text-red-500"}`}>{Number(ticker?.priceChange) > 0 ? "+" : ""} {ticker?.priceChange} {Number(ticker?.priceChangePercent)?.toFixed(2)}%</p>
                        </div>
                        <div className="flex flex-col">
